        ...
        if not request.POST.get('mention') == '':
            ...
            for i in range(1, len(mentioned_users_id_list)):
                ...
                if user.profile.level == 'bluemover':
                    ...
                    subject = user.last_name + user.first_name + "님, " + comment_writer.last_name + comment_writer.first_name + "님의 댓글에 멘션되셨습니다."
                    message_text = \
                        """
                        <!doctype html>
                        <html
...
def update(request, id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            notice_channel_id = request.POST.get('channel_id').split('#')[0]
            notice_channel_name = request.POST.get('channel_id').split('#')[1]
            for i in range(notice.favorite_count):
                favorite_user_id = notice.favorite_user_set.values()[i].get('id')
                ...
                if user.profile.level == 'bluemover':
                    ...
                    client.chat_postMessage(
                        ...
                        blocks = [
                            {
                                ...
                                "text": {
                                    "type": "plain_text",
                                    "text": "📑 북마크에 있는 '" + notice.title + "' 업데이트됨",
                                }
                            },
                            {
                                "type": "section",
                                "text": {
                                    "type": "mrkdwn",
                                    "text": "<@" + user.email.replace('@bluemove.or.kr', '').lower() + ">님, 공지사항 '" + notice.title + "'의 업데이트를 확인하세요."
                                }
                            },
                            {
                                "type": "section",
                                "text": {
                                    "type": "mrkdwn",
                                    "text": "```대상: " + notice_category + "\n제목: " + notice_title + "\n작성자: " + notice.writer.last_name + notice.writer.first_name + "\n내용: " + textwrap.shorten(re.sub(pattern='<[^>]*>', repl='', string=form.data['content']).replace('&nbsp;', ' '), width=100) + "```"
                                }
                            },
                            {
                                "type": "section",
                                ...
                                "accessory": {
                                    "type": "image",
                                    "image_url": SocialAccount.objects.filter(user=request.user)[0].extra_data['picture'],
                                    "alt_text": notice.writer.last_name + notice.writer.first_name + "님의 프로필 사진"
                                }
                            },
                            {
                                "type": "actions",
                        ...
                        text = f"📑 북마크에 있는 '" + notice.title + "' 업데이트됨",
                    )
                else:
                    # 01. 서비스 계정 Gmail API 호출
                    INSIDE_CLIENT = 'docs@bluemove.or.kr'
                    ...
                    sender = 'Bluemove Docs ' + '<' + INSIDE_CLIENT + '>' ##### INSIDE 클라이언트 이메일 주소 INPUT #####
                    to = user.email ##### 멘션된 유저 이메일 주소 INPUT #####
                    subject = user.last_name + user.first_name + "님, " + notice.title + "의 업데이트를 확인하세요."
                    message_text = \
                        """
                        <!doctype html>
                        <html
                                                                                                ...
                                                                                                <tbody>
                                                                                                    <tr>
                                                                                                        <td
                                                                                                            ...
                                                                                                            class="mcnTextContent"
                                                                                                            style="padding: 18px;color: #58595B;font-family: Helvetica;font-size: 14px;font-weight: normal;">
                                                                                                            <strong style="color:#222222;">대상</strong>: """ + notice_category + """<br>
                                                                                                            <strong style="color:#222222;">제목</strong>: """ + notice_title + """<br>
                                                                                                            <strong style="color:#222222;">작성자</strong>: """ + notice.writer.last_name + notice.writer.first_name + """<br>
                                                                                                            <strong style="color:#222222;">내용</strong>: """ + textwrap.shorten(re.sub(pattern='<[^>]*>', repl='', string=form.data['content']).replace('&nbsp;', ' '), width=100) + """<br>
                                                                                                            <strong style="color:#222222;">수정일</strong>: """ + str(notice.updated_at) + """
                                                                                                        </td>
                                                                                                    </tr>
                                                                                                </tbody>
                                                                                            </table>
